#!/usr/bin/env python3
"""
Generate the NERONOTES mod logo (square), in the shared Neroland family style
...
the music mod, so the core-prism radiates ELECTRIC CYAN sound-wave rings with sparkle motes. Renders supersampled, then downsamples.

Outputs:
  art/logo/neronotes_logo.png       (1024x1024 master)
  art/logo/neronotes_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/neronotes_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 53
MOD = 'neronotes'
NAME = 'NERONOTES'
ACCENT = (36, 210, 255)
ACCENT_BRIGHT = (140, 240, 255)
BRIGHT = (232, 252, 255)
NEBULA = (20, 64, 104)
STAR_TINT = (200, 240, 255)
NAME_GLOW = (36, 210, 255, 255)
TAG_COL = (182, 226, 240, 255)
...
TAG = 'C O M P O S E   ·   P R E S S   ·   S Y N C'
...
def emblem_frame(base, cx, cy, rad):
    ...
    for i in range(n):
        ...
        long = (i % 2 == 0)
        r0 = rad * 1.18
        r1 = rad * (2.0 if long else 1.55)
        ww = rad * (0.16 if long else 0.09)
        pa = a + math.pi / 2
        ...
        col = ACCENT if long else STARSTEEL
        gd.polygon([tip, b1, b2], fill=col + (235,))
    base.alpha_composite(gl.filter(ImageFilter.GaussianBlur(SS)))
    base.alpha_composite(gl)
    # sparkle motes
    ml = Image.new("RGBA", (R, R), (0, 0, 0, 0)); md = ImageDraw.Draw(ml)
    for t, rr in ((30, 1.75), (150, 1.7), (255, 1.8), (330, 1.55)):
        a = math.radians(t); mx, my = cx + math.cos(a) * rad * rr, cy + math.sin(a) * rad * rr
        s = rad * 0.12
        md.line([mx - s, my, mx + s, my], fill=ACCENT_BRIGHT + (255,), width=SS * 2)
        md.line([mx, my - s, mx, my + s], fill=ACCENT_BRIGHT + (255,), width=SS * 2)
